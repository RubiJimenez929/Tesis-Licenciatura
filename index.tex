...
\chapter{Modelo Matemático de Programación Dinámica Estocástica para
Optimizar la Operación de un Sistema
Hidroeléctrico.}\label{modelo-matemuxe1tico-de-programaciuxf3n-dinuxe1mica-estocuxe1stica-para-optimizar-la-operaciuxf3n-de-un-sistema-hidroeluxe9ctrico.}

En este capítulo se presenta una formulación del problema de
optimización de la operación de un sistema hidroeléctrico mediante un
Proceso de Decisión de Markov (MDP), usando Programación Dinámica
Estocástica (PDE) con el fin de obtener la política óptima de operación
tomando en cuenta el concepto de curva guía. Lo anterior se propone tal
y como lo describe Díaz (2010), quien aplica la PDE para optimizar la
operación del sistema hidroeléctrico del río Grijalva. Se enfatiza la
conexión entre los elementos matemáticos formales y su implementación
numérica.

\section{Datos históricos}\label{datos-histuxf3ricos}

Sea \(T = 50\) los años históricos disponibles \((1959–2008)\),
indexados por \(t \in \{0,\dots,T-1\}\). Cada uno de ellos se
subdividieron en \(q = 24\) quincenas (correspondientes a las columnas
de los datos históricos proporcionados por Díaz (2010)).
...
\(V_{i,t,q} \in \mathbb{R}_{+} \cup \{0\}\) (Millones de metros
cúbicos-\(Mm^3\)).

\section{Formulación del Proceso de Decisión de Markov
(MDP)}\label{formulaciuxf3n-del-proceso-de-decisiuxf3n-de-markov-mdp}

Conforme a Puterman (1994), se define el MDP como la colección de
objetos \[\{(T, \mathcal{S}, \mathcal{A}, \mathbb{P}, \mathcal{R})\},\]
donde cada componente se describe a continuación.

\subsection{Estructura temporal: quincenas y etapas
hidrológicas}\label{estructura-temporal-quincenas-y-etapas-hidroluxf3gicas}

Para reducir la dimensionalidad temporal del problema y poder capturar
la estacionalidad, las \(24\) quincenas se agrupan en \(M = 6\) etapas
hidrológicas (segmentación temporal del año) mediante la siguiente
función de agregación \(\mu: \{0,\dots,q-1\} \to \{0,\dots,M-1\}\)
definida por
\begin{equation}\protect\phantomsection\label{eq-etapas}{\mu(q) = \begin{cases} 0, & 0 \leq q <10 & \text{Ene-Mayo}\\ 1, & 10 \leq q <14 & \text{Jun-Jul}\\ 2, & 14 \leq q < 16 & \text{Agosto}\\ 3, & 16 \leq q < 18 & \text{Septiembre}\\ 4, & 18 \leq q < 20 & \text{Octubre}\\ 5, & 20 \leq q \leq 23 & \text{Nov-Dic}\\ \end{cases}}\end{equation}
así, la variable \(m = \mu(q)\) representará la etapa hidrológica
correspondiente a la quincena \(q\).

\subsection{\texorpdfstring{Espacio de estados y operador de
discretización
\((\mathcal{S})\)}{Espacio de estados y operador de discretización (\textbackslash mathcal\{S\})}}\label{espacio-de-estados-y-operador-de-discretizaciuxf3n-mathcals}

Se introduce un operador de discretización uniforme \(\mathcal{D}\) del
espacio de estados \(\mathcal{S}\) mediante el parámetro
\(\Delta = 600\) \(Mm^3\) (paso de discretización). Dicha unidad permite
discretizar el volumen almacenado y la afluencia agregada por etapa, así
como también, subdividir la capacidad de almacenamiento de cada presa en
un número manejable de estados. Para un volumen continuo \(V_{i,t,q},\)
se define el operador de discretización
...
donde \(N_1 = 27\) y \(N_2 = 16\) son las capacidades discretas máximas
de cada presa. El volumen máximo representado en rejilla discreta es
\begin{equation}\protect\phantomsection\label{eq-volumen-maximo}{V_{i,\max} = N_i \Delta,}\end{equation}
\(16,200\) \(Mm^3\) para Angostura y \(9,600\) \(Mm^3\) para Malpaso.

Así, el espacio de estados se define como
\[\mathcal{S} = \mathcal{S}_1 \times \mathcal{S}_2 \times \{0,...,5\} = \{s = (s_1,s_2,m)\},\]
en donde \((s_1,s_2,m)\) representa el volumen discretizado de
Angostura, el volumen discretizado de Malpaso y la etapa hidrológica
correspondiente a la quincena \(q\).

\subsection{\texorpdfstring{Espacio de acciones
\((\mathcal{A})\)}{Espacio de acciones (\textbackslash mathcal\{A\})}}\label{espacio-de-acciones-mathcala}

Para cada etapa \(m\), se define una unidad de extracción de agua
\(u_m = \{100, 200, 300, 300, 400, 250\}\), así la decisión de operación
...
\(u_m\), la cual está en \(Mm^3,\) para obtener el volumen de agua
extraído en cada presa. Por lo tanto, el espacio de acciones se define
como \[\mathcal{A}_m = \mathcal{K}_{1,m} \times \mathcal{K}_{2,m},\]
donde \(\mathcal{K}_{1,m} = \mathcal{K}_{2,m}\) representan el conjunto
de niveles de turbinado disponibles para ambas presas en la etapa
hidrológica \(m\). En la práctica, tanto la Angostura como Malpaso
tienen el mismo conjunto de niveles de turbinado y mismas unidades de
extracción por etapa.

Una acción denotada por \(a = (k_1, k_2)\), produce el volumen real
turbinado \(k_i\cdot u_m\) aplicado simultaneamente en ambas presas. Por
ejemplo, suponga que \(m = 0\) entonces
\[u_0 = 100 \quad \text{y} \quad \mathcal{K}_{1,0}, \mathcal{K}_{2,0} = \{0,1,...,20\},\]
y así \(a = (3,5)\) significaría que Angostura turbina
\(3\times 100 = 300\) \(Mm^3\) de agua y Malpaso turbina
\(5\times 100 = 500\) \(Mm^3\) de agua. Cabe mencionar que Los valores
de \(u_m\) y \(\mathcal{K}_{i,m}\) se seleccionaron para que el
turbinado máximo por etapa hidrológica \(k_{i} \cdot u_m\) no exceda la
capacidad instalada de las plantas ni los caudales históricos máximos
registrados, garantizando la factibilidad física de todas las acciones
evaluadas.
...
antes de tomar la acción de turbinado, \(W_{i, t,q+1}\) representa el
afluente neto (caudal de ingreso) para la presa \(i\) en la quincena
\(q+1\) del año \(t\), tales valores serán obtenidos de los datos
históricos con los que ya se cuentan. Por último \(k_iu_m\),
\(k_i \in \mathcal{K}_{i,m},\) representa el volumen de agua extraído
por la acción de turbinado aplicada en la etapa hidrológica
correspondiente a la quincena \(q\).
...
En ambos casos, el volumen discretizado de la quincena \(q+1\) se
obtiene aplicando el operador de discretización \(\mathcal{D}\) al
volumen resultante, es decir, \[s_i' = \mathcal{D}(V_{i,t,q+1}, N_i),\]
mientras que la etapa hidrológica evoluciona mediante
\(m' = (m+1) \mod M\).
...
\((\mathcal{R})\)}{Función de recompensa (\textbackslash mathcal\{R\})}}\label{funciuxf3n-de-recompensa-mathcalr}
...
\begin{equation}\protect\phantomsection\label{eq-altura}{H_i (s_i,s'_i) =\frac{20(s_i + s_i'+2)}{2} =10 (s_i + s_i'+2).}\end{equation}
...
en estados con bajo volumen y el término \(20\) como intercepto que
representa la altura hidráulica mínima mientras que como pendiente
representa cuántos metros de altura se ganan por cada incremento en el
estado discretizado, es decir, la ganancia por cada incremento de
\(\Delta\) en el volumen de agua almacenado.
...
donde \(10^6\) representa la conversión de \(Mm^3\) a \(m^3\) y
\(3600\times 10^6\) la conversión de \(kWh\) a \(GWh\).

Por lo tanto la energía total generada por el sistema es
...
\item
  Penalización por niveles críticos (casi desbordamiento): si
  \(s_i' \geq N_i-1\) se aplica
...
penalización ajustadas empíricamente con valores
\(10000, 10000, 5000, 20000\) respectivamente y \(\text{CG}_i(m)\)
representa la curva guía para la presa \(i\) en la etapa \(m\).

La función de recompensa total se define finalmente por
...
trayectorias históricas \[(s_t, a_t, r_t, s_{t+1}')\] en donde cada una
de ellas representa una experiencia de transición observada en los datos
históricos, siendo \(r_t\) el valor obtenido de la
Equation~\ref{eq-recompensa}, representando la recompensa obtenida al
tomar la acción \(a_t\) en el estado \(s_t\) y transicionar a
\(s_{t+1}'\). Este enfoque se alinea con un esquema de
\textbf{\emph{Batch Reinforcement Learning.}}

\section{Ecuación de optimalidad de
Bellman}\label{ecuaciuxf3n-de-optimalidad-de-bellman}

Conforme a la teoría clásica de control estocástico, por ejemplo
Puterman (1994), Teorema 6.2.3, en un MDP descontado existe una única
función de valor de acción óptima
\(V^*:\mathcal{S}\times\mathcal{A} \to \mathbb{R}\) que satisface la
ecuación de optimalidad de Bellman cuando los conjuntos de estados y
acciones son finitos:
\begin{equation}\protect\phantomsection\label{eq-bellman}{V^*(s,a) = \mathcal{R}(s,a,s') + \gamma \sum_{s'\in \mathcal{S}} P(s'|s,a) \max_{a' \in \mathcal{A}} V^*(s',a').}\end{equation}

Dado que en lugar de una función de transición analítica se cuenta con
un dataset empírico de trayectorias \((s_t, a_t, r_t, s_{t+1}'),\) para
todo \(t,\) la ecuación de Bellman se reformula en su versión empírica
como
\begin{equation}\protect\phantomsection\label{eq-bellman-empirica}{V^*(s,a) = \mathcal{R}(s,a,s') + \gamma \max_{a' \in \mathcal{A}} V^*(s',a'),}\end{equation}

para más detalles consultar Ernst et al. (2005).
...
Si bien es sabido que la programación dinámica estocástica está afectada
por la maldición de la dimensionalidad, Castelletti et al. (2010),
mientras más variables de estado hayan, el requisito computacional crece
exponencialmente, además de que se debe contar con un modelo de
transición del sistema explícito, por ello el presente trabajo se basa
en un enfoque de aproximación funcional mediante Fitted Q-Iteration
(FQI), el cual es un algoritmo de aprendizaje por refuerzo que permite
aproximar la función de valor de acción óptima \(V^*\) a partir de un
conjunto de transiciones empíricas.

\subsection{Iteración de Bellman
empírica}\label{iteraciuxf3n-de-bellman-empuxedrica}

Al principio, se supone una función de valor de acción inicial
\(V^{(0)}\) (\(V^{(0)}(s,a) = 0\) para todo \((s,a)\)). Luego,
sucesivamente se generan nuevas aproximaciones para cada iteración
\(n\), actualizando la función de valor de acción mediante la ecuación
de Bellman empírica. Para cada transición observada
\((s_t, a_t, r_t, s_{t+1}')\) en el dataset, se define el
\textbf{objetivo de Bellman} por
\begin{equation}\protect\phantomsection\label{eq-objetivo-bellman}{y^{(n)} := r + \gamma \max_{a' \in \mathcal{A}} V^{(n-1)}(s',a'),}\end{equation}
el cual representa una estimación del valor de acción óptimo asociado al
par \((s,a)\) basada en la recompensa obtenida \(r\) y el valor de
acción máximo en el estado siguiente \(s'\) según la función de valor de
acción de la iteración anterior \(V^{(n-1)}\).
...
Equation~\ref{eq-objetivo-bellman}, se ajusta una función de valor de
acción \(V^{(n)}\) que aproxime la relación entre los pares \((s,a)\) y
los valores objetivos \(y^{(n)}\), dicha función se obtiene resolviendo
un problema de aprendizaje supervisado que se explicó en la sección
(pendiente).

El procedimiento se formula como un problema de regresión no
paramétrica, en donde se busca encontrar una función \(V^{(n)}\) que
minimice el error entre las predicciones \(V^{(n)}(s,a)\) y los
objetivos \(y^{(n)}\) para todas las transiciones
\((s_t, a_t, r_t, s_{t+1}')\) en el dataset. En la práctica, se utiliza
el algoritmo de \emph{ExtraTrees}, que como propone Ernst et al. (2005),
el cual argumenta que ofrece gran flexibilidad para aproximar funciones
de valor en espacios continuos o de alta dimensionalidad.

La modelación mediante ExtraTrees compuesto por \(300\) árboles de
decisión, con una profundidad máxima de \(14\) y un mínimo de \(5\)
muestras por hoja. Cada árbol de decisión particiona el espacio
estado-acción en regiones homogéneas, (grupo de \((s,a)\) similares) y
proporciona una estimación local de la función de valor. La predicción
final corresponde al promedio de las predicciones individuales de todos
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{Imagenes/arbol-decision.png}

}

\caption{\label{fig-arbol}Árbol de decisión con profundidad máxima de 14
y un mínimo de 5 muestras por hoja.}

\end{figure}%

El algoritmo de FQI se repite iterativamente hasta que la función de
valor de acción converge. En la práctica se definió una métrica de
convergencia empírica por
\begin{equation}\protect\phantomsection\label{eq-convergencia}{\Delta = \frac{1}{L}\sum_{l=1}^L |V^{(n)}(s_l,a_l) - V^{(n-1)}(s_l,a_l)|,}\end{equation}
la cual se define sobre
\(\mathcal{T} \subset \{(s_t, a_t, r_t, s_{t+1}')\}\) siendo \(L\) la
cardinalidad de \(\mathcal{T}\).

Por lo que el algoritmo se detiene hasta que el \(\Delta < \epsilon\),
es decir, cuando el cambio entre \(V^{(n)}\) y \(V^{(n-1)}\) sea menor a
un umbral predefinido o bien cuando se alcance un número máximo de
iteraciones.

\section{Política óptima y simulación
operativa}\label{poluxedtica-uxf3ptima-y-simulaciuxf3n-operativa}

Una vez obtenida la función de valor de acción óptima \(V^*,\) se deriva
la política óptima \(\pi^*:\mathcal{S} \to \mathcal{A}\) que asigna a
cada estado \(s\) la acción \(a\) que maximiza el valor de acción
\begin{equation}\protect\phantomsection\label{eq-politica-optima}{\pi^*(s) := \arg\max_{a \in \mathcal{A}} V^*(s,a),}\end{equation}

es decir, para cada estado \(s\), la política óptima \(\pi^*(s)\) indica
la acción que se debe tomar para maximizar el valor de acción estimado.

Finalmente, se valida la política óptima \(\pi^*\) mediante simulación
en lazo cerrado, es decir, se simula la operación del sistema
hidroeléctrico utilizando la política \(\pi^*\) para tomar decisiones de
turbinado en cada estado observado durante las trayectorias históricas.

La evaluación de la política se realiza mediante un recorrido
determinista sobre la serie histórica completa. El algoritmo opera de la
siguiente manera:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Inicialización:} Se fijan los volúmenes iniciales \(V_{1,0}\)
  y \(V_{2,0}\).
\item
  \textbf{Mapeo estacional:} Para cada quincena \(q\), se determina
  \(m = \mu(q)\) y la unidad de extracción \(u_m\).
\item
...
\item
  ...
  \]
\item
  \textbf{Registro y actualización:} Se almacenan métricas de energía,
  derrames y volúmenes, y se actualiza el estado para la siguiente
  iteración.
\end{enumerate}

La metodología sigue la práctica estándar en aprendizaje por refuerzo
aplicado a sistemas de control documentado en Bertsekas (2019), en el
cual la política aprendida se valida mediante una simulación en lazo
cerrado sobre trayectorias históricas.
...
Una vez obtenidos los resultados de la simulación en lazo cerrado, se
evalúa el desempeño de la política óptima \(\pi^*\) utilizando métricas
clave como las siguientes:

\begin{longtable}[]{@{}llll@{}}
\caption{Resumen del desempeño energético del sistema bajo la política
óptima aprendida mediante FQI. Los valores corresponden al promedio
sobre el periodo de simulación
1959--2008.}\label{tbl-desempeno-energetico}\tabularnewline
\toprule\noalign{}
...
Por otro lado, los derrames promedio quincenales y anuales se presentan
...
óptima aprendida mediante FQI. Los valores corresponden al promedio
sobre el periodo de simulación
1959--2008.}\label{tbl-desempeno-hidrico}\tabularnewline
\toprule\noalign{}
Métrica & Angostura & Malpaso & Sistema Total \\
\midrule\noalign{}
\endfirsthead
...
De acuerdo con los resultados obtenidos y con la información obtenida de
Díaz (2010), se observa que la política no está gestionando
adecuadamente los recursos hídricos, ya que el volumen de agua derramada
es considerablemente alto, lo que sugiere que la política aprendida no
está optimizando eficientemente el uso del agua disponible para
generación energética. Además, la contribución energética de cada presa
al sistema total no refleja un balance adecuado, lo que podría indicar
que la política está favoreciendo excesivamente una presa sobre la otra,
lo que no es deseable desde el punto de vista de la gestión integrada
del sistema hidroeléctrico. Estos resultados sugieren que la política
óptima aprendida mediante FQI no está logrando un equilibrio adecuado
entre la generación de energía y la gestión eficiente del recurso
hídrico, lo que podría requerir una revisión de la formulación del
problema, la función de recompensa o la metodología de aprendizaje
utilizada para mejorar el desempeño del sistema en términos de ambos
objetivos.
...
\chapter{Modelo Matemático Fitted Q-Iteration para Optimizar la
Operación de un Sistema
Hidroeléctrico.}\label{modelo-matemuxe1tico-fitted-q-iteration-para-optimizar-la-operaciuxf3n-de-un-sistema-hidroeluxe9ctrico.}
...
algoritmo de FQI (Section~\ref{sec-conjunto-aprendizaje}), la afluencia
se agrega a nivel de \emph{etapa} (\(M=6\) decisiones por año) y no se
recorre la serie quincena por quincena; (ii) para \textbf{desplegar y
evaluar la política} ya aprendida (Section~\ref{sec-simulacion}), la
decisión sí se reconsulta en cada una de las 24 quincenas del año,
usando en cada una la afluencia real de esa quincena, aunque los
parámetros vigentes (capacidad máxima, curva guía, grilla de acción)
sean los de la etapa \(m=\mu(q)\) que la contiene.
...
heredados de la discretización de la PDE precedente, no se usan aquí
para discretizar el estado, sino únicamente para fijar las
\textbf{capacidades máximas continuas} de cada presa,
\begin{equation}\protect\phantomsection\label{eq-capacidad-maxima}{V_{1,\max} := N_1 \Delta = 16{,}200  \text{Mm}^3, \quad V_{2,\max} := N_2 \Delta = 9{,}600 \text{Mm}^3,}\end{equation}
y para convertir a volumen la curva guía, definida originalmente en
unidades de índice (Section~\ref{sec-recompensa}).
...
&= [0,V_{1,\max}]\times[0,V_{2,\max}]
   \times\{0,\dots,5\} = \{s=(s_1,s_2,m)\},
\end{aligned}
}\end{equation} donde \[s_i\in[0,V_{i,\max}],\ i=1,2,\ 
m\in\{0,\dots,5\}.\]

\textbf{Grilla gruesa de referencia.} Aunque el estado en sí no se
discretiza, tanto la construcción del conjunto de aprendizaje
(Section~\ref{sec-conjunto-aprendizaje}) como la aproximación del
operador de maximización (Section~\ref{sec-max-operador}) requieren
...
\end{aligned}
\] (idéntico para ambas presas en cada etapa, es decir,
\(\mathcal K_{1,m} = \mathcal K_{2,m} = \mathcal K_{m}\)). Estos índices
\textbf{no enumeran directamente el conjunto de acciones factibles del
FQI}: se usan únicamente para obtener la cota superior del turbinado
factible en la etapa,
\begin{equation}\protect\phantomsection\label{eq-accion-max}{
...
turbinar \(300\) y \(500\) Mm³ respectivamente, dentro de la cota, pero
el FQI no restringe la acción a estos múltiplos enteros de \(u_m\)).
...
continuo. Al igual que con el estado, para fines de muestreo
(Section~\ref{sec-conjunto-aprendizaje}) y de maximización aproximada
(Section~\ref{sec-max-operador}) este intervalo se discretiza en una
grilla gruesa de \(N_a=9\) puntos equiespaciados,
...
\subsection{\texorpdfstring{Dinámica de transición
\((\mathbb{P})\)}{Dinámica de transición (\textbackslash mathbb\{P\})}}\label{dinuxe1mica-de-transiciuxf3n-mathbbp-1}

\textbf{Balance de masa a nivel de etapa (usado para el conjunto de
aprendizaje).} Para construir las transiciones de entrenamiento
...
histórica completa (Section~\ref{sec-simulacion}), la decisión se toma
en cada quincena \(q\), usando el afluente histórico de esa
...
}\end{equation} con manejo de frontera anual, si \(q=23\), entonces\\
\begin{equation}\protect\phantomsection\label{eq-transicion23}{
V_{i,t+1,0} = V_{i,t,23} + W_{i,t,23} - a_i.
}\end{equation}

En ambos casos \(a_i\) es el volumen turbinado decidido para la etapa
\(m=\mu(q)\) vigente en esa quincena
...
Desde el punto de vista de Puterman (1994), la dinámica del MDP se
resume en el núcleo de transición \(\mathbb P(\cdot\,|\,s,a)\), que en
este modelo \textbf{no se especifica en forma analítica cerrada}: toda
la incertidumbre proviene de la afluencia histórica \(\xi_{i,t,m}\) (o
...
\subsubsection{Modelo hidráulico-energético
linealizado}\label{modelo-hidruxe1ulico-energuxe9tico-linealizado-1}
...
\subsubsection{Estructura de
penalizaciones}\label{estructura-de-penalizaciones-1}
...
\section{Ecuación de optimalidad de
Bellman}\label{ecuaciuxf3n-de-optimalidad-de-bellman-1}
...
\section{Aproximación numérica mediante Fitted Q-Iteration
(FQI)}\label{aproximaciuxf3n-numuxe9rica-mediante-fitted-q-iteration-fqi-1}
...
\subsection{Iteración de Bellman
empírica}\label{iteraciuxf3n-de-bellman-empuxedrica-1}
...
\subsection{Aproximación funcional por regresión no paramétrica
(ExtraTrees)}\label{aproximaciuxf3n-funcional-por-regresiuxf3n-no-paramuxe9trica-extratrees-1}
...
capítulo}\label{conclusiones-del-capuxedtulo-1}
...
\bibitem[\citeproctext]{ref-Alegria2010}
Díaz, Arallely Alegría. 2010. {``Política de Operación Óptima Del
Sistema de Presas Del Río Grijalva. Efectos de La Curva Guía.''} Tesis
de Maestría en Ingeniería Civil -\/- Hidráulica, Universidad Nacional
