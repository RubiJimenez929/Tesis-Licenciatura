...
lang: es
---
# Modelo Matemático de Programación Dinámica Estocástica para Optimizar la Operación de un Sistema Hidroeléctrico.

::: {style="text-align: justify"}
En este capítulo se presenta una formulación del problema de optimización de la operación de un sistema hidroeléctrico mediante un Proceso de Decisión de Markov (MDP), usando Programación Dinámica Estocástica (PDE) con el fin de obtener la política óptima de operación tomando en cuenta el concepto de curva guía. Lo anterior se propone tal y como lo describe @Alegria2010, quien aplica la PDE para optimizar la operación del sistema hidroeléctrico del río Grijalva. Se enfatiza la conexión entre los elementos matemáticos formales y su implementación numérica.
:::
## Datos históricos
::: {style="text-align: justify"}
Sea $T = 50$ los años históricos disponibles $(1959–2008)$, indexados por $t \in \{0,\dots,T-1\}$. Cada uno de ellos se subdividieron en $q = 24$ quincenas (correspondientes a las columnas de los datos históricos proporcionados por @Alegria2010). 

El sistema está compuesto por dos presas: [La Angostura](https://maps.app.goo.gl/NHNQdHdfYAoRGxhPA) $(i = 1)$ y [Malpaso](https://maps.app.goo.gl/acX7hVXswX5uYYUBA) $(i =2)$. El volumen de almacenamiento de cada una de ellas en la quincena $q$ del año $t$ se representará por $V_{i,t,q} \in \mathbb{R}_{+} \cup \{0\}$ (Millones de metros cúbicos-$Mm^3$).

## Formulación del Proceso de Decisión de Markov (MDP)

Conforme a @puterman1994markov, se define el MDP como la colección de objetos $$\{(T, \mathcal{S}, \mathcal{A}, \mathbb{P}, \mathcal{R})\},$$ donde cada componente se describe a continuación.

:::
### Estructura temporal: quincenas y etapas hidrológicas
::: {style="text-align: justify"}
Para reducir la dimensionalidad temporal del problema y poder capturar la estacionalidad, las $24$ quincenas se agrupan en $M = 6$ etapas hidrológicas (segmentación temporal del año) mediante la siguiente función de agregación $\mu: \{0,\dots,q-1\} \to \{0,\dots,M-1\}$ definida por $$\mu(q)=
\begin{cases}
0, & 0\le q<10 \quad(\text{Ene–May})\\
1, & 10\le q<14 \quad(\text{Jun–Jul})\\
2, & 14\le q<16 \quad(\text{Ago})\\
3, & 16\le q<18 \quad(\text{Sep})\\
4, & 18\le q<20 \quad(\text{Oct})\\
5, & 20\le q\le 23 \quad(\text{Nov–Dic})
\end{cases}$$ {#eq-etapas}
...
::: {style="text-align: justify"}
Se introduce un operador de discretización uniforme $\mathcal{D}$ del espacio de estados $\mathcal{S}$ mediante el parámetro $\Delta = 600$ $Mm^3$ (paso de discretización). Dicha unidad permite discretizar el volumen almacenado y la afluencia agregada por etapa, así como también, subdividir la capacidad de almacenamiento de cada presa en un número manejable de estados. Para un volumen continuo $V_{i,t,q},$ se define el operador de discretización $\mathcal{D}: \mathbb{R} \times \mathbb{N} \longrightarrow \{0,\dots,N-1\}$ dado por $$s_i = \mathcal{D}(V_{i,t,q}, N_i) := \min \left(\max\left(\left\lfloor \frac{V_{i,t,q}}{\Delta} + \frac{1}{2}\right\rfloor, 0\right), N_i-1\right),$$ {#eq-discretizacion}
donde $N_1 = 27$ y $N_2 = 16$ son las capacidades discretas máximas de cada presa. El volumen máximo representado en rejilla discreta es $$V_{i,\max} = N_i \Delta,$$ {#eq-volumen-maximo}
$16,200$ $Mm^3$ para Angostura y $9,600$ $Mm^3$ para Malpaso.

Así, el espacio de estados se define como $$\mathcal{S} = \mathcal{S}_1 \times \mathcal{S}_2 \times \{0,...,5\} = \{s = (s_1,s_2,m)\},$$ en donde $(s_1,s_2,m)$ representa el volumen discretizado de Angostura, el volumen discretizado de Malpaso y la etapa hidrológica correspondiente a la quincena $q$.
:::
#### Discretización del proceso de afluencia
::: {style="text-align: justify"}
A diferencia de la simulación que usa la afluencia histórica $V_{i,t,q}$ (quincenal), la fase de planeación, trabaja con la afluencia agregada por etapa hidrológica $m$, esto se debe a que cada paso de la ecuación de Bellman corresponde a una etapa completa. Así, para la presa $i$, en el año $t$ y la etapa $m$, se define la afluencia agregada como $$W_{i,t,m} = \sum_{q:\mu(q)=m} W_{i,t,q},$$ {#eq-discretizacion-afluencia}  
:::

### Espacio de acciones $(\mathcal{A})$
::: {style="text-align: justify"}
Para cada etapa $m$, se define una unidad de extracción de agua $u_m = \{100, 200, 300, 300, 400, 250\}$, así la decisión de operación en la etapa $m$ corresponderá a los niveles de turbinado $k_1, k_2$. Cada nivel de turbinado se multiplica por la unidad de extracción $u_m$, la cual está en $Mm^3,$ para obtener el volumen de agua extraído en cada presa. Por lo tanto, el espacio de acciones se define como $$\mathcal{A}_m = \mathcal{K}_{1,m} \times \mathcal{K}_{2,m},$$
donde $\mathcal{K}_{1,m} = \mathcal{K}_{2,m}$ representan el conjunto de niveles de turbinado disponibles para ambas presas en la etapa hidrológica $m$. En la práctica, tanto la Angostura como Malpaso tienen el mismo conjunto de niveles de turbinado y mismas unidades de extracción por etapa.

Una acción denotada por $a = (k_1, k_2)$, produce el volumen real turbinado $k_i\cdot u_m$ aplicado simultaneamente en ambas presas. Por ejemplo, suponga que $m = 0$ entonces $$u_0 = 100 \quad \text{y} \quad \mathcal{K}_{1,0}, \mathcal{K}_{2,0} = \{0,1,...,20\},$$ y así $a = (3,5)$ significaría que Angostura turbina $3\times 100 = 300$ $Mm^3$ de agua y Malpaso turbina $5\times 100 = 500$ $Mm^3$ de agua. Cabe mencionar que Los valores de $u_m$ y $\mathcal{K}_{i,m}$ se seleccionaron para que el turbinado máximo por etapa hidrológica $k_{i} \cdot u_m$ no exceda la capacidad instalada de las plantas ni los caudales históricos máximos registrados, garantizando la factibilidad física de todas las acciones evaluadas.
:::

### Dinámica de transición $(\mathbb{P})$
::: {style="text-align: justify"}
La ecuación de transición continua tras la operación del sistema se define por $$V_{i,t,q+1} = V_{i,t,q} + W_{i, t, q}-k_iu_m, \quad q<23$$ {#eq-transicion}
donde $V_{i,t,q}$ representa el volumen almacenado en la presa $i$ antes de tomar la acción de turbinado, $W_{i, t,q}$ representa el afluente neto (caudal de ingreso) para la presa $i$ en la quincena $q$ del año $t$, tales valores serán obtenidos de los datos históricos con los que ya se cuentan. Por último $k_iu_m$, $k_i \in \mathcal{K}_{i,m},$ representa el volumen de agua extraído por la acción de turbinado aplicada en la etapa hidrológica correspondiente a la quincena $q$.

Cuando $q=23$, la transición se define por $$V_{i,t+1,0} = V_{i,t,23} + W_{i, t, 23}-k_iu_m.$$ {#eq-transicion23}

En ambos casos, el volumen discretizado de la quincena $q+1$ se obtiene aplicando el operador de discretización $\mathcal{D}$ al volumen resultante, es decir, $$s_i' = \mathcal{D}(V_{i,t,q+1}, N_i),$$
...
Tal expresión corresponde a un modelo lineal basado en estados discretizados, el término $+2$ se introdujo para evitar alturas nulas en estados con bajo volumen y el término $20$ como intercepto que representa la altura hidráulica mínima mientras que como pendiente representa cuántos metros de altura se ganan por cada incremento en el estado discretizado, es decir, la ganancia por cada incremento de $\Delta$ en el volumen de agua almacenado.
...
- Penalización por niveles críticos (casi desbordamiento): si $s_i' \geq N_i-1$ se aplica $$\Pi_{crit} = C_{crit}$$ {#eq-penalizacion-niveles-criticos}

donde $C_{derr}, C_{def}, C_{CG}, C_{crit}$ son constantes de penalización ajustadas empíricamente con valores $10000, 10000, 5000, 20000$ respectivamente y $\text{CG}_i(m)$ representa la curva guía para la presa $i$ en la etapa $m$.
...
Dado que en lugar de una función de transición analítica se cuenta con un dataset empírico de trayectorias $(s_t, a_t, r_t, s_{t+1}'),$ para todo $t,$ la ecuación de Bellman se reformula en su versión empírica como $$V^*(s,a) = \mathcal{R}(s,a,s') + \gamma \max_{a' \in \mathcal{A}} V^*(s',a'),$$ {#eq-bellman-empirica}
...
El algoritmo de FQI se repite iterativamente hasta que la función de valor de acción converge. En la práctica se definió una métrica de convergencia empírica por $$\Delta = \frac{1}{L}\sum_{l=1}^L |V^{(n)}(s_l,a_l) - V^{(n-1)}(s_l,a_l)|,$${#eq-convergencia} la cual se define sobre $\mathcal{T} \subset \{(s_t, a_t, r_t, s_{t+1}')\}$ siendo $L$ la cardinalidad de $\mathcal{T}$.
